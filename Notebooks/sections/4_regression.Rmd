...
interact_all <- d[, 
                 lm(clicked ~ textvisual + 
                      ...
                      I(text * male) +
                      I(visual * male) +
                      I(textvisual * unknown_gender) +
                      I(text * unknown_gender) +
                      I(visual * unknown_gender))]
...
# Interact with removed unknown gender and unknown age
basic_m <- d[, lm(clicked ~ textvisual + 
                  text + 
                  visual)]
...
stargazer(basic_m, interact_m, interact_all,
          se = c(
            list(robust_se(basic_m)),
            list(robust_se(interact_m)),
            list(robust_se(interact_all))
            ),
          type = 'text')
...
```{r grouped and interaction terms}
#Kevin's code
m_no_interact <- lm(clicked ~ factor(group) + factor(age) + factor(gender), data = d)
m_with_intrct <- lm(clicked ~ I(group) + I(age) + I(gender) + (I(group) * I(gender)), data = d)
stargazer(m_no_interact,
          m_with_intrct,
          se=c(list(robust_se(m_no_interact)),
               list(robust_se(m_with_intrct))),
          type = "text")
```
...
tret <- lm(clicked ~ I(group), data=d)
stargazer(tret, full,
          se=c(list(robust_se(tret)),
               list(robust_se(full))), 
          type = "text")
```